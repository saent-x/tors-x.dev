...
<nav class={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScroll ? 'bg-white/50 backdrop-blur-lg shadow-sm dark:shadow-white/20 dark:bg-dark-theme' : ''} `}>
    <a href="#top" aria-label="navbar">
        {#if theme.isDarkMode && theme.isInitialized}
            <enhanced:img class="w-15 cursor-pointer mr-14" src={logo_white} alt="logo"/>
        {:else}
            <enhanced:img class="w-15 cursor-pointer mr-14" src={logo} alt="logo"/>
        {/if}
    </a>

    <ul class={`hidden md:flex items-center gap-6 lg:gap-8 rounded-full px-12 py-3 ${isScroll ? '' : 'bg-white/50 shadow-sm dark:border dark:border-white/50 dark:bg-transparent'}`}>
    ...
    <div class="flex items-center gap-4">
        ...
        <a class="font-ovo hidden lg:flex items-center gap-3 px-10 py-2.5 border border-gray-500 rounded-full ml-4 dark:border-white/50" href="#contact">
            Contact
    ...
    <!-- Mobile Menu -->
    <ul bind:this={sideMenuRef} class="flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500 dark:bg-dark-hover dark:text-white">
